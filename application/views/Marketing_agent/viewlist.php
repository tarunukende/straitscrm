			...
			<div class="box">
				<div class="box-header with-border">
					...
					<div class="pull-right">
						<a class="btn btn-primary" href="<?php echo base_url().'marketing_agent/add' ?>"> <i class="fa fa-plus"></i> Add</a>
					</div>
				</div>
				<div class="box-body">
						...
						<tbody>
							<?php if ($Records): ?>
								<?php foreach ($Records as $Record): ?>
									<?php $Leader = $this->Marketingagent_model->getById('marketing', $Record['team_lead_id']) ?>
									<tr>
										<td><?php echo $Record['id'] ?></td>
										<td><?php echo $Record['name'] ?></td>
										<td><?php echo $Record['email'] ?></td>
										<td><?php echo $Record['mobile_number'] ?></td>
										<td><?php echo ($Leader) ? $Leader['name'] : 'No Region' ?></td>
										<!-- <td><?php echo $Record['status'] ?></td> -->
										<td><?php echo ($Record['status']) ? 'Enabled' : 'Disabled' ; ?></td>
										<td class="text-center">
											<a href="<?php echo base_url()."marketing_agent/edit/".$Record['id'] ?>" data-toggle="tooltip" data-placement="top" title="Edit">
												<i class="fa fa-edit"></i>
											</a>&nbsp&nbsp
											<a class="delete" href="javascript:void(0);" data-id="<?php echo($Record['id']) ?>" data-toggle="tooltip" data-placement="bottom" title="Delete"> <i class="fa fa-trash"></i>
											</a>&nbsp&nbsp
		...
		$(document).ready(function()
		{
			...
			$('.delete').click(function()
			{
				...
				if (confirm("Do you want to delete this Record?")==true)
					window.location = url+"marketing_agent/delete/"+id;
				else
					return false;
			})
			...
			$('.enable').click(function()
			{
				...
				if (confirm("Do you want to enable this Record?")==true)
					window.location = url+"marketing_agent/enable/"+id;
				else
					return false;
			})
			...
			$('.disable').click(function()
			{
				...
				if (confirm("Do you want to disable this Record?")==true)
					window.location = url+"marketing_agent/disable/"+id;
				else
					return false;
			})
		});
