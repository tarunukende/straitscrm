...
//        this.callNotification();
    ...
    callNotification: function () {
        ...
        salesexecuetiveHelper.title = value;
    },
...
//        }, 200);
        ...
        $.ajax({
            ...
            success: function (returndata) {
                salesexecuetiveHelper.createNotification(returndata);
                salesexecuetiveHelper.updateNotification(returndata);
            },
            beforeSend: function () {
//                helAddEvent.enableWaitCursor();
        ...
        $.each(eventObj, function (index, value) {
            ...
            salesexecuetiveHelper.updateLeadStatus(value.id, "0");
...
//                    helAddEvent.updateNotification(returndata);
        ...
        $.each(eventObj, function (index, value) {
            salesexecuetiveHelper.setupdatedPopup(value);
        });
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function (returndata) {
                salesexecuetiveHelper.setExecutiveList(returndata);
            },
            beforeSend: function () {
//                helAddEvent.enableWaitCursor();
            },
    ...
    setExecutiveList: function (resource) {
        ...
        if (Object.keys(resource).length > 0) {
            $.each(resource, function (key, value) {
                var vstatus = "";
                if (value.id == sales_id) {
                    vstatus = "selected";
                }
                $('.executive-for-drp').append($('<option ' + vstatus + '>', {value: value.id}).text(value.name));
            });
        }
        ...
        $.ajax({
            ...
            {
                salesexecuetiveHelper.updateLeadStatus(leadid, "2");
                location.reload();
            }
        });
        ...
        $('#leadsdata').DataTable({
            ...
            "ajax": {
                ...
                complete: function () {
                    salesexecuetiveHelper.getExecutive();
                }
            },
            "columns": [
                {"data": "id"},
...
$(document).on('change', '.target', function () {
    salesexecuetiveHelper.assignLeadtoExe(this);
});
$(document).on('click', '.details', function () {
    salesexecuetiveHelper.viewLeadDetails(this);
});
...
$(document).ready(function () {
    salesexecuetiveHelper.setbaseURL();
    salesexecuetiveHelper.setleadlist();
    salesexecuetiveHelper.init();
    setInterval(function () {
        salesexecuetiveHelper.getNotification();
    }, 20000);
});